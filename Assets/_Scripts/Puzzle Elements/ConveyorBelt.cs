...
namespace GravityGame.Puzzle_Elements
{
    public class ConveyorBelt : MonoBehaviour
    {
        ...
        [SerializeField] float _textureSpeed;
        ...
        void Update() => _material.mainTextureOffset += new Vector2(1, 0) * (_textureSpeed * Time.deltaTime);

        void FixedUpdate()
        {
            for (int i = 0; i <= _onBelt.Count - 1; i++) {
                _onBelt[i].AddForce(_speed * _direction, ForceMode.Acceleration);
            }
        }
