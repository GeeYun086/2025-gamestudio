...
namespace GravityGame.Player
{
    ...
    public class PlayerCarry : MonoBehaviour
    {
        ...
        public Transform HiddenCarryPoint;
        ...
        public float ObstructedTimeUntilMovedToHiddenCarry = 0.5f;
        ...
        public void AttemptPickUp(Carryable carryable)
        {
            ...
            if (!carryable || carryable.Rigidbody.mass > MaxCarryMass) return;

            _carry.Object = carryable;
            _carry.Object.SetGravityEnabled(false);
            IgnorePlayerCollision(_carry.Object.gameObject, true);
        }

        public void AttemptRelease()
        {
            if (!_carry.Object) return;
            if (_carry.IsTargetObstructed) return;

            IgnorePlayerCollision(_carry.Object.gameObject, false);
            _carry.Object.SetGravityEnabled(true);
            _carry.Object.Collider.enabled = true;
            _carry.Object = null;
        }
        ...
        void Update()
        {
            ...
            if (_carry.Object) {
                _carry.Object.Collider.enabled = true;
                if (FindObstructedCarryPosition() is { } pos) {
                    _carry.IsTargetObstructed = IsOverlappingWithPlayer(pos);
                    Debug.Log($"obstructed: {_carry.IsTargetObstructed}");
                } else {
                    _carry.IsTargetObstructed = false;
                }
                
                if (_carry.IsTargetObstructed) {
                    _carry.Position = HiddenCarryPoint.position;
                    _carry.Object.Collider.enabled = false;
                }
        ...
        Vector3 GetCarryPosition()
        {
            ...
            float clampedLookDownAngle = Mathf.Clamp(_camera.LookDownRotation, MinAngle, MaxAngle);
        ...
        Vector3? FindObstructedCarryPosition()
        {
            ...
            var hitCount = Physics.BoxCastNonAlloc(start, halfExtents, direction.normalized, results, _carry.Rotation, direction.magnitude, layerMask);
