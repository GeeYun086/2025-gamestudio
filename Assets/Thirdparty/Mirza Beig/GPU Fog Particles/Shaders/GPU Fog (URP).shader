    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(VertexOutput IN) : SV_Target
            {
                ...
                float2 texCoord3 = IN.ase_texcoord3.xy * float2(1, 1) + ((_SimpleNoiseAnimation * _TimeParameters.x) + (
                    ParticleStableRandom43 * 10.0));
                float simpleNoise1 = SimpleNoise(texCoord3 * _SimpleNoiseScale);
                float SimpleNoise18 = saturate(
                    (0.0 + (simpleNoise1 - _SimpleNoiseRemap) * (1.0 - 0.0) / (1.0 - _SimpleNoiseRemap)));
                float lerpResult34 = lerp(1.0, SimpleNoise18, _SimpleNoiseAmount);
                float2 texCoord20 = IN.ase_texcoord3.xy * float2(1, 1) + float2(0, 0);
                float simplePerlin3D19 = snoise(
                    (float3(texCoord20, 0.0) + (_SimplexNoiseAnimation * _TimeParameters.x) + (ParticleStableRandom43 *
                        20.0)) * _SimplexNoiseScale);
                simplePerlin3D19 = simplePerlin3D19 * 0.5 + 0.5;
                float SimplexNoise25 = saturate(
                    (0.0 + (simplePerlin3D19 - _SimplexNoiseRemap) * (1.0 - 0.0) / (1.0 - _SimplexNoiseRemap)));
                ...
                float2 texCoord7 = IN.ase_texcoord3.xy * float2(1, 1) + ((_VoronoiNoiseAnimation).xy * _TimeParameters.
                    x);
                float2 coords2 = texCoord7 * _VoronoiScale;
                float2 id2 = 0;
                float2 uv2 = 0;
                float voroi2 = voronoi2(coords2, time2, id2, uv2, 0, voronoiSmoothId2);
                float VoronoiNoise12 = saturate(
                    (0.0 + (voroi2 - _VoronoiNoiseRemap) * (1.0 - 0.0) / (1.0 - _VoronoiNoiseRemap)));
                float lerpResult5 = lerp(1.0, VoronoiNoise12, _VoronoiNoiseAmount);
                float Noise36 = (lerpResult34 * lerpResult33 * lerpResult5);
                float RemappedNoise60 = saturate(
                    (0.0 + (Noise36 - _CombinedNoiseRemap) * (1.0 - 0.0) / (1.0 - _CombinedNoiseRemap)));
                ...
                float distanceDepth50 = abs(
                    (screenDepth50 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (_SurfaceDepthFade));
                float SurfaceDepthFade55 = saturate(distanceDepth50);
                float eyeDepth = IN.ase_texcoord5.x;
                float cameraDepthFade87 = ((eyeDepth - _ProjectionParams.y - _CameraDepthFadeOffset) /
                    _CameraDepthFadeRange);
                ...
                float Alpha = saturate(
                    (RemappedNoise60 * SurfaceDepthFade55 * CameraDepthFade88 * RadialMask79 * Albedo81.a));
        ...
        Pass
        {
            ...
            half4 frag(VertexOutput IN) : SV_TARGET
            {
                ...
                float2 texCoord3 = IN.ase_texcoord2.xy * float2(1, 1) + ((_SimpleNoiseAnimation * _TimeParameters.x) + (
                    ParticleStableRandom43 * 10.0));
                float simpleNoise1 = SimpleNoise(texCoord3 * _SimpleNoiseScale);
                float SimpleNoise18 = saturate(
                    (0.0 + (simpleNoise1 - _SimpleNoiseRemap) * (1.0 - 0.0) / (1.0 - _SimpleNoiseRemap)));
                ...
                float time2 = mulTime6;
                float2 voronoiSmoothId2 = 0;
                float2 texCoord7 = IN.ase_texcoord2.xy * float2(1, 1) + ((_VoronoiNoiseAnimation).xy * _TimeParameters.
                    x);
                float2 coords2 = texCoord7 * _VoronoiScale;
                float2 id2 = 0;
                float2 uv2 = 0;
